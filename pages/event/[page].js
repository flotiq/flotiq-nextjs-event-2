...
export async function getStaticProps({params}) {
    ...
    const moreEvents = replaceUndefinedWithNull(await getEvents(1, 3, filtersMoreEvents))
    const nextEvents = replaceUndefinedWithNull(await getEventNext(event.data[0].date))
    const prevEvents = replaceUndefinedWithNull(await getEventPrevious(event.data[0].date))
